...
import type { InitOptions, DetectedDependency } from '../lib/types.js';
import {
  readPackageJson,
  detectProjectType,
  detectDependencies,
...
export async function initCommand(options: InitOptions): Promise<void> {
  ...
  if (!packageJson) {
    ...
    console.log(chalk.dim('Please run this command in a Node.js project directory.'));
    return;
  }
  ...
  if (!options.noDetect) {
    ...
    if (detected.length === 0) {
      ...
      for (const dep of detected) {
        ...
        const version = dep.framework?.name ? getMajorVersion(dep.version) : dep.version;

        console.log(`  ${chalk.dim('├──')} ${frameworkName}@${dep.version} → ${frameworkName.toLowerCase()}@${version} ${templateStatus}`);
      }
