...
/**
 * Mask API key for display (security: never show full key in console)
 */
...
export async function authCommand(options: AuthOptions): Promise<void> {
  ...
  if (options.status) {
    ...
    if (availability.mcp) {
      console.log(
        chalk.dim("  MCP fallback: available (Claude Code session detected)")
      );
    }
    return;
  }
  ...
  if (options.logout) {
    ...
    if (config.apiKey) {
      ...
      config.configuredAt = undefined;
      await writeGlobalConfig(config);
      console.log(chalk.green("✓ API key removed from config"));
      console.log(
        chalk.dim(
          "  Note: Also unset CONTEXT7_API_KEY from your environment if set"
        )
      );
    } else {
  ...
  console.log(
    chalk.dim("Context7 provides up-to-date documentation for frameworks.")
  );
  ...
  const response = await prompts([
    {
      ...
      validate: (value) => {
        ...
        if (!value.startsWith("ctx7")) {
          return "Invalid API key format (should start with ctx7)";
        }
        return true;
      },
    },
