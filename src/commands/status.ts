...
import type { StatusResult } from '../lib/types.js';
import {
  readConfig,
  configExists,
  readPackageJson,
...
export async function statusCommand(): Promise<void> {
  ...
  const installedDeps = packageJson
  ...
  // Frameworks section
  console.log(chalk.bold('Frameworks') + chalk.dim(` (${Object.keys(config.frameworks).length}):`));
  ...
  if (frameworkEntries.length === 0) {
    ...
    for (let i = 0; i < frameworkEntries.length; i++) {
      ...
      const installedVersion = installed ? getMajorVersion(installed.version) : null;
  ...
  console.log('');
  console.log(chalk.bold('Internal Patterns') + chalk.dim(` (${config.internal.categories.length} categories):`));
  ...
  if (internalCategories.length === 0) {
    ...
    for (let i = 0; i < internalCategories.length; i++) {
      ...
      const prefix = isLast ? '└──' : '├──';
      const categorySize = files.reduce((sum, f) => sum + f.sizeBytes, 0) / 1024;
  ...
  const indexStatus = percentUsed > 90
  ...
  const docsStatus = docsPercent > 90
  ...
  if (missingFrameworks.length > 0) {
    console.log('');
    console.log(chalk.yellow('Missing frameworks (installed but not documented):'));
    for (const dep of missingFrameworks) {
      console.log(`  ${chalk.dim('└──')} ${dep.framework!.displayName}@${getMajorVersion(dep.version)}`);
    }
