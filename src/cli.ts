...
program
  ...
  .argument(
    "[frameworks...]",
    "Framework names to add (e.g., hono drizzle zod)"
  )
...
program
  ...
  .action((options) => {
        ...
        const priority =
          t.priority === "P0"
            ...
            : t.priority === "P1"
              ...
              : chalk.dim("●");
        console.log(
          `  ${priority} ${t.name.padEnd(16)} ${chalk.dim(t.version.padEnd(6))} ${t.displayName}`
        );
      }
      console.log("");
    }
    ...
    console.log(
      chalk.dim(
        "Priority: ● P0 (essential)  ● P1 (recommended)  ● P2 (optional)"
      )
    );
    console.log("");
  });
