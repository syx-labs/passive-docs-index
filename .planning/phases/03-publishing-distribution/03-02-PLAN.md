...
must_haves:
  truths:
    - "Merging changesets to main automatically creates a Version Packages PR with bumped version and CHANGELOG"
    - "Merging a version PR to main triggers npm publish with cryptographic provenance"
    - "Published package on npmjs.com shows a provenance badge linking to the GitHub Actions build"
    - "No npm tokens or secrets are needed after initial OIDC setup -- publishing is fully automated"
    - "Build artifacts (JS bundles + .d.ts) are freshly built in CI before every publish"
  artifacts:
...
<task type="auto">
  ...
  2. Create `.github/workflows/publish.yml` -- npm publish with OIDC provenance:
     ...
     NOTE on CI build compatibility: The publish workflow runs `bun run build` which executes
     the same two-step build (build:js + build:types) configured in 03-01. The `bun install`
     step installs typescript (devDependency), ensuring `tsc -p tsconfig.build.json` works
     identically to local. The artifact verification step after build catches any CI-specific
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  **If first publish fails:**
  - `E403 Forbidden` -- Package name may be taken. Check `npm view passive-docs-index` first. If taken, you need to scope the package (`@syx-labs/passive-docs-index`) and update package.json name + publish.yml accordingly.
  - `E401 Unauthorized` -- Token is invalid or expired. Re-generate at npmjs.com -> Access Tokens. Ensure the token has "Read and Write" permission.
  - `EPUBLISHCONFLICT` -- This version already exists on npm. Bump version in package.json before retrying.
  - Network/timeout errors -- Retry. npm publish is idempotent for the same version (second attempt for same version will get EPUBLISHCONFLICT, confirming first attempt succeeded).
  ...
  **If OIDC setup fails or is unavailable:**
  - "Trusted Publishers" section not visible -- Ensure you are logged in as the package owner/maintainer, not just an org member.
  - OIDC publish fails after configuration -- Check that the workflow filename matches exactly (`publish.yml`), repository owner/name are correct, and environment is blank. Check GitHub Actions logs for the specific OIDC error.
  - Fallback: If OIDC cannot be configured, add an `NPM_TOKEN` secret to the GitHub repo (Settings -> Secrets -> Actions -> New) and modify publish.yml to use `NODE_AUTH_TOKEN: ${{ secrets.NPM_TOKEN }}` in the publish step. This works but requires token rotation every 90 days.
