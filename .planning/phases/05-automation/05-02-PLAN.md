...
<task type="auto">
  <name>Task 2: Postinstall hook implementation and tests</name>
  <files>
    src/lib/postinstall.ts
    tests/unit/lib/postinstall.test.ts
  </files>
...
A lightweight script that checks for PDI config and reports staleness to stderr.

**Decision note — `node ./dist/postinstall.js` vs `npx pdi status`:**
CONTEXT.md Decision #4 says "Use npx/bunx: Postinstall script uses `npx pdi status`". However, research (05-RESEARCH.md) found that for the package's OWN postinstall script, `node ./dist/postinstall.js` is technically superior:
- No npx resolution overhead (npx must resolve, potentially download, then execute)
- No risk of npx interactive prompts blocking install
- Direct node execution is faster (critical: postinstall must be < 2 seconds)
- The dist/ directory is already part of the installed package — no PATH issues
The SPIRIT of the decision (works without global install) IS fully honored: `node ./dist/postinstall.js` runs the package's own bundled code directly, requiring neither global install nor PATH resolution. This approach is strictly better than npx for this use case.

Must follow these rules per research and locked decisions:
...
bunx tsc --noEmit
```
All postinstall tests pass and types check.
  </verify>
  <done>
- postinstall.ts writes freshness hints to stderr, never exits non-zero, handles all errors silently
- All postinstall tests pass covering discovery hint, freshness display, error resilience, and stderr-only output
  </done>
</task>

<task type="auto">
  <name>Task 3: Build config, package.json postinstall entry, and index exports</name>
  <files>
    package.json
    src/index.ts
  </files>
...
1. Add postinstall script: `"postinstall": "node ./dist/postinstall.js"`
   (Using `node ./dist/postinstall.js` per decision intent — see Task 2 decision note for rationale. This runs the package's own bundled code directly, no npx overhead, no PATH issues.)
2. Update build:js script to ALSO build `src/lib/postinstall.ts` -> `dist/postinstall.js`:
   ```
   ...
   ```
3. `"dist/postinstall.js"` is already covered by `"files": ["dist"]` so no change needed there.

**Update `src/index.ts`:**

Add exports for the new modules created in plans 05-01 and 05-02:
```typescript
// Registry Client
export {
  fetchLatestVersion,
...
**Note:** Document Bun trustedDependencies limitation in the summary (not in code).
  </action>
  <verify>
```bash
bunx tsc --noEmit
